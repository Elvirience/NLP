# считывание с фотографий документов текстовых данных по их координатам
...
for i in range(len(data)):
    ...
    for j in marks:
        ...
        try: text = reader.readtext(crop, detail=0)
        except: continue
        # entities.append( {j['entityId']: text} )
        if text: entities.append( [j['entityId'], text] )
